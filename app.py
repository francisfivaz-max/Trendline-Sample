...
st.caption("Data loads from a GitHub RAW URL. Always-on red target line shows the max target.")

left, right = st.columns([1,4], gap="large")

with left:
    st.subheader("Data (URL only)")
    gh_url = st.text_input("GitHub RAW Excel URL", placeholder="https://raw.githubusercontent.com/you/repo/branch/path.xlsx")
    st.info("Paste the **GitHub RAW** URL to your Excel. Uploads are disabled by design.")
...
if "raw_data" not in st.session_state or refresh:
    if gh_url:
        try:
            st.session_state["raw_data"] = load_data_from_url(gh_url)
