# Water Quality Trends — Monthly Trends (Read‑only) — Always show target line
...
def load_excel() -> pd.DataFrame:
    if EXCEL_URL:
        ...
        if content[:20].lstrip().startswith(b"<"):
            raise RuntimeError("EXCEL_URL did not return binary Excel. Use RAW link.")
...
st.caption("Monthly values = last test per month per Site ID. Target line always shown.")
