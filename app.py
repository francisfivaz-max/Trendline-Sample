...
# v6.5: deep diagnostics for Result parsing and column detection
...
def parse_result(x):
    ...
    low = s.lower()
    if low in {"nd","n/d","not detected","bdl","bdl.","tntc","na","n/a","--","-"}: return 0.0
    # remove thousands separator like 1,234 or 1 234
    s = s.replace(" ", "")
...
def load_excel():
    ...
    long["Result"]   = long["Result_raw"].map(parse_result)
...
# Sidebar: Type and Type-specific month range
with st.sidebar:
    st.header("Filters")
