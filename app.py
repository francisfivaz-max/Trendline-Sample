...
# Area removed, Type kept. Robust date parsing & string trimming.
...
def load_excel() -> pd.DataFrame:
    ...
    if has_date and has_sample:
        ...
        df["DateClean"] = pd.NaT  # nothing we can do
...
# Monthly = last reading per month per site (use DateClean for ranking)
if not f.empty:
    rank_col = "DateClean" if "DateClean" in f.columns else ("Date" if "Date" in f.columns else None)
    if rank_col is not None:
        f["_rank"] = f.groupby(["Site ID","MonthStart"])[rank_col].rank(method="first", ascending=False)
        monthly = f[f["_rank"] == 1.0].copy()
    else:
        # No usable date column: just take the mean per month/site to show something
...
st.caption("Monthly values = last test per month per Site ID (by Date or Sample date). Max target is solid red. Area removed, Type kept.")
