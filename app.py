...
# v3: adds diagnostics, refresh button, and clearer empty-state handling
...
def load_excel_from_sources(excel_url: str) -> pd.DataFrame:
    # --- Load ---
    if excel_url:
        ...
        for name in ["Results", "Final", "Monthly", "Trends", "Sheet1", "Data"]:
            if name in df:
                data = df[name].copy()
                break
        else:
        ...
        if not os.path.exists(LOCAL_XLSX):
            # Return empty frame with expected columns so UI renders
            return pd.DataFrame(columns=["Type", "Site ID", "Parameter", "Unit", "Result", "DateRaw"])
    ...
    if keep:
        ...
        data = pd.DataFrame(columns=["Type", "Site ID", "Parameter", "Unit", "Result", "DateRaw"])
    ...
    # Parse date + numeric
    if "DateRaw" in data.columns:
        data["DateClean"] = data["DateRaw"].map(parse_date_any)
    else:
        data["DateClean"] = pd.NaT
    if "Result" in data.columns:
        data["Result"] = data["Result"].map(parse_result_to_float)
    else:
        data["Result"] = None
    ...
    # Basic defaults
    for col in ["Type", "Parameter", "Site ID", "Unit"]:
...
@st.cache_data(show_spinner=False)
def load_targets(targets_url: str) -> pd.DataFrame | None:
    try:
        if targets_url:
            content = _http_get(targets_url)
            tdf = pd.read_csv(io.BytesIO(content))
        else:
            if not os.path.exists(LOCAL_TARGETS):
                return None
...
# Load data
# -----------------------------
data = load_excel_from_sources(EXCEL_URL)
...
param_col, site_col = st.columns([2,2])
with param_col:
    parameter_sel = st.selectbox("Parameter", params) if params else None
with site_col:
    # Site IDs (based on param selection if exists; else general)
    if parameter_sel:
...
# Date range widget (robust when subset is empty)
if "MonthStart" in subset0.columns and subset0["MonthStart"].notna().any():
...
with st.expander("Data diagnostics (for current selection)"):
    ...
    st.write("**Row count:**", len(data))
    if "Parameter" in data.columns:
        st.write("**Unique Parameters (sample 30):**", sorted(data["Parameter"].dropna().astype(str).unique().tolist())[:30])
    if "Type" in data.columns:
        st.write("**Types:**", sorted(data["Type"].dropna().astype(str).unique().tolist()))
    if "Site ID" in data.columns:
        st.write("**Site IDs (sample 30):**", sorted(data["Site ID"].dropna().astype(str).unique().tolist())[:30])
    st.dataframe(data.head(20))

# If we don't have a parameter list, guide the user and stop early
if not params:
    st.error(
        "No **Parameter** values were found in the loaded data. "
        "Please verify that your Excel file includes a 'Parameter' column and that **EXCEL_URL** is set in Streamlit Secrets. "
        "Use the diagnostics above to confirm columns."
    )
    st.stop()
...
if tdf is not None and parameter_sel is not None and not pd.isna(parameter_sel):
    trow = tdf.loc[tdf["Parameter"].astype(str) == str(parameter_sel)]
