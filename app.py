...
for c in df.columns:
    ...
    elif lc in {"result","results","value","reading"} or "result" in lc: colmap[c] = "Result"
...
date_like = [c for c in df.columns if c.lower().strip() in {"date"} or ("date" in c.lower()) or ("sample" in c.lower())]
if len(date_like) == 0:
    st.error("Could not find any date-like column. Please ensure your data has a 'Date' column or one containing 'date' or 'sample'.")
    st.stop()
# If multiple, take first non-null from left to right
date_block = df[date_like].copy()
# Convert each to datetime independently, then coalesce
for c in date_block.columns:
    date_block[c] = pd.to_datetime(date_block[c], errors="coerce")
df["Date"] = date_block.bfill(axis=1).iloc[:,0]
# Drop any extra duplicate Date columns we may have created via rename
extra_dates = [c for c in df.columns if c != "Date" and c.lower() == "date"]
if extra_dates:
    df.drop(columns=extra_dates, inplace=True, errors="ignore")
