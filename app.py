# Water Quality Trends — Monthly Trends (Read‑only) — GitHub RAW source (fixed loader)
...
def load_excel() -> pd.DataFrame:
    if EXCEL_URL:
        ...
        if head.startswith(b"<!DOCTYPE") or head.startswith(b"<html"):
            raise RuntimeError("EXCEL_URL did not return a binary .xlsx. Use a RAW link (raw.githubusercontent.com), not a GitHub page.")
...
def load_targets() -> pd.DataFrame:
    if TARGETS_URL:
        ...
        head = content[:200].lstrip()
        if head.startswith(b"<!DOCTYPE") or head.startswith(b"<html"):
            st.warning("TARGETS_URL looks like a webpage. Use a RAW CSV link.")
        else:
            t = pd.read_csv(io.BytesIO(content))
            t.columns = [c.strip() for c in t.columns]
            if "Parameter" in t.columns and "MaxTarget" in t.columns:
                return t[["Parameter","MaxTarget"]]
            st.warning("Targets CSV missing required columns Parameter, MaxTarget. Falling back to local.")
